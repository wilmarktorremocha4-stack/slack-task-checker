...
const STATUS = {
  ...
  cancelled:          { label: "Cancelled",     badge: "bg-slate-500/20 text-slate-400 border-slate-500/30",      dot: "bg-slate-500" },
} as const;
...
// Per-tab color styles for inactive state — dark-friendly
const FILTER_COLORS: Record<string, { tab: string; badge: string }> = {
  all:                 { tab: "bg-white/[0.05] border-white/[0.10] text-white/60 hover:bg-white/[0.09] hover:border-white/[0.18]",              badge: "bg-white/10 text-white/50" },
  pending_review:      { tab: "bg-amber-500/10 border-amber-500/25 text-amber-300 hover:bg-amber-500/20 hover:border-amber-500/40",              badge: "bg-amber-500/15 text-amber-300" },
  active:              { tab: "bg-blue-500/10 border-blue-500/25 text-blue-300 hover:bg-blue-500/20 hover:border-blue-500/40",                   badge: "bg-blue-500/15 text-blue-300" },
  revision_requested:  { tab: "bg-orange-500/10 border-orange-500/25 text-orange-300 hover:bg-orange-500/20 hover:border-orange-500/40",         badge: "bg-orange-500/15 text-orange-300" },
  completed:           { tab: "bg-emerald-500/10 border-emerald-500/25 text-emerald-300 hover:bg-emerald-500/20 hover:border-emerald-500/40",    badge: "bg-emerald-500/15 text-emerald-300" },
  escalated:           { tab: "bg-rose-500/10 border-rose-500/25 text-rose-300 hover:bg-rose-500/20 hover:border-rose-500/40",                   badge: "bg-rose-500/15 text-rose-300" },
  cancelled:           { tab: "bg-white/[0.04] border-white/[0.08] text-white/40 hover:bg-white/[0.07] hover:border-white/[0.14]",               badge: "bg-white/[0.06] text-white/35" },
};
...
// ── Surface styles: dark glass cards ─────────────────────────────────────────

const CARD = "bg-white/[0.04] backdrop-blur-xl border border-white/[0.08] shadow-xl shadow-black/40";
const CARD_HOVER = "hover:bg-white/[0.06] hover:border-white/[0.14] hover:shadow-2xl hover:shadow-black/50";
const INPUT_CLS = "w-full bg-white/[0.07] border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white/80 placeholder-white/25 hover:border-white/20 focus:outline-none focus:ring-2 focus:ring-indigo-400/30 focus:border-indigo-400/40 transition-all";
const HEADER_BTN = "bg-white/[0.08] backdrop-blur-sm border border-white/[0.12] hover:bg-white/[0.14] text-white/80 shadow-lg";
...
function ConfirmDialog({ title, body, confirmLabel, danger, onConfirm, onClose }: {
  ...
  return createPortal(
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[#0d1b2e]/95 backdrop-blur-xl border border-white/10 rounded-2xl w-full max-w-sm p-6 anim-pop shadow-2xl shadow-black/60">
        <h3 className="text-base font-semibold text-white/90 mb-2">{title}</h3>
        <p className="text-sm text-white/40 mb-6 leading-relaxed">{body}</p>
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 bg-white/[0.07] border border-white/10 hover:bg-white/[0.12] text-white/70 font-medium text-sm py-2.5 rounded-xl transition-all active:scale-[0.98]">
            Keep as is
          </button>
          <button
            onClick={() => { onConfirm(); onClose(); }}
            className={`flex-1 font-semibold text-sm py-2.5 rounded-xl text-white transition-all active:scale-[0.98] shadow-lg ${
              danger ? "bg-rose-600 hover:bg-rose-700 shadow-rose-500/25" : "bg-indigo-600 hover:bg-indigo-700 shadow-indigo-500/25"
            }`}
          >
            {confirmLabel}
          </button>
  ...
  if (c.author_type === "system") {
    return (
      <div className="flex justify-center my-1">
        <span className="text-xs text-white/40 italic px-3 py-1 bg-white/[0.05] rounded-full text-center border border-white/[0.07]">
  ...
  return (
    <div className={`flex gap-2 ${isBrandon ? "flex-row-reverse" : "flex-row"}`}>
      <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${
        isBrandon ? "bg-indigo-500 text-white" : "bg-white/10 text-white/70"
      }`}>
        {c.author_name.charAt(0).toUpperCase()}
      </div>
      <div className={`max-w-[75%] ${isBrandon ? "items-end" : "items-start"} flex flex-col`}>
        <span className="text-xs text-white/30 mb-1 px-1">{c.author_name} · {timeAgo(c.created_at)}</span>
        <div className={`px-3.5 py-2 rounded-2xl text-sm leading-relaxed ${
          isBrandon
            ? "bg-indigo-500/80 text-white rounded-tr-sm shadow-md shadow-indigo-500/20"
  ...
  return (
    <div
      className={`rounded-2xl transition-all duration-200 ${CARD} ${CARD_HOVER} ${
        isPendingReview ? "ring-2 ring-amber-400/40 border-amber-500/30" : ""
      }`}
    ...
    >
      ...
      {/* Card header */}
      <button onClick={onToggle} className="w-full text-left p-5 flex items-start gap-4 rounded-2xl transition-colors hover:bg-white/[0.03]">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1.5 flex-wrap">
            ...
            {isPendingReview && <span className="text-xs font-medium text-amber-300/80">Awaiting your review</span>}
            {replyCount > 0 && (
              <span className="inline-flex items-center gap-1 text-xs font-medium text-indigo-300 bg-indigo-500/15 border border-indigo-500/30 px-2 py-0.5 rounded-full">
          ...
          <p className="font-semibold text-white/90 leading-snug">{task.task_text}</p>
          <p className="text-sm text-white/40 mt-1 flex items-center gap-1.5 flex-wrap">
            {accentColor && <span className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: accentColor }} />}
            <span className="text-white/70 font-medium">{assigneeDisplay}</span>
            <span className="text-white/20">·</span>
            assigned by {task.assigned_by_name}
          </p>
          <p className="text-xs text-white/30 mt-0.5">{formatDate(task.created_at)} · {timeAgo(task.created_at)}</p>
        </div>

        <div className="text-right shrink-0 flex flex-col items-end gap-1">
          <span className="text-xs text-white/30">{task.followup_count}/{task.max_followups} follow-ups</span>
          {next && (
            <span className={`text-xs font-medium ${next.isOverdue ? "text-rose-400" : "text-indigo-300"}`}>
              {next.label}
            </span>
          )}
          ...
          <svg className={`w-4 h-4 text-white/30 mt-1 transition-transform ${expanded ? "rotate-180" : ""}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="m6 9 6 6 6-6" />
          </svg>
        </div>
      </button>
      ...
      {expanded && (
        <div className="border-t border-white/[0.07] px-5 pb-5 pt-4 anim-expand">
          {/* Thread history */}
          <div className="mb-4">
            <p className="text-xs text-white/30 font-semibold uppercase tracking-widest mb-3">── Activity</p>
            {sortedComments.length > 0 ? (
              <div ref={threadRef} className="flex flex-col gap-3 max-h-72 overflow-y-auto pr-1 hide-scrollbar bg-white/[0.02] border border-white/[0.06] rounded-xl p-3">
                {sortedComments.map(c => <CommentBubble key={c.id} c={c} userMap={userMap} />)}
              </div>
            ) : (
              <p className="text-sm text-white/30 italic text-center py-4 bg-white/[0.02] rounded-xl border border-white/[0.06]">
          ...
          {isPendingReview && (
            <div className="space-y-3">
              <div className="flex items-center gap-2 text-sm text-amber-300 font-medium bg-amber-500/10 border border-amber-500/25 rounded-xl px-4 py-2.5">
                <span>👀</span>
              ...
              <button disabled={!!working} onClick={() => setConfirm("cancel")} className="w-full inline-flex items-center justify-center gap-2 bg-white/[0.05] border border-rose-500/30 hover:bg-rose-500/10 text-rose-400 font-medium text-sm py-2.5 rounded-xl transition-all disabled:opacity-50 active:scale-[0.98]">
          ...
          {isOpen && (
            <div className="space-y-3">
              <div className="flex gap-3">
                <button disabled={!!working} onClick={() => setConfirm("followup_now")} className="flex-1 inline-flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 hover:-translate-y-0.5 disabled:opacity-50 text-white font-semibold text-sm py-2.5 rounded-xl shadow-lg shadow-indigo-500/25 transition-all active:scale-[0.98]">
                  <IconBolt /> {working === "followup_now" ? "Sending..." : "Send Follow-up Now"}
                </button>
                <button disabled={!!working} onClick={() => setConfirm("cancel")} className="flex-1 inline-flex items-center justify-center gap-2 bg-white/[0.05] border border-rose-500/30 hover:bg-rose-500/10 text-rose-400 font-medium text-sm py-2.5 rounded-xl transition-all disabled:opacity-50 active:scale-[0.98]">
                ...
                <div className="pt-3 border-t border-white/[0.07]">
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 hover:-translate-y-0.5 disabled:opacity-40 text-white font-semibold text-sm px-4 py-2 rounded-xl shadow-md shadow-indigo-500/20 transition-all active:scale-[0.98]"
                    >
          ...
          {/* Closed task actions */}
          {isClosed && (
            <button disabled={!!working} onClick={() => setConfirm("reopen")} className="w-full inline-flex items-center justify-center gap-2 bg-white/[0.05] border border-indigo-500/30 hover:bg-indigo-500/10 text-indigo-300 font-medium text-sm py-2.5 rounded-xl transition-all disabled:opacity-50 active:scale-[0.98]">
...
function EmployeeView({ tasks, expandedId, onToggle, onAction, userMap, readState }: {
  ...
  if (employees.length === 0) {
    return (
      <div className={`text-center text-white/40 py-16 ${CARD} rounded-2xl`}>
      ...
      {employees.map(([name, empTasks]) => {
        ...
        return (
          <div key={name} className={`rounded-2xl ${CARD} overflow-hidden`}
            ...
            {/* Employee header */}
            <div className="px-5 py-4 border-b border-white/[0.07]" style={{ background: color + "18" }}>
              <div className="flex items-start justify-between gap-4 flex-wrap">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-white/90 text-base flex items-center gap-2">
                      {name}
                      <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: color }} />
                    </h3>
                    <p className="text-xs text-white/40">{total} task{total !== 1 ? "s" : ""} total</p>
                  </div>
                </div>

                {/* Status pill row */}
                <div className="flex gap-2 text-xs flex-wrap">
                  {active > 0    && <span className="px-2.5 py-1 rounded-full bg-blue-500/20 text-blue-300 border border-blue-500/30">{active} active</span>}
                  {pending > 0   && <span className="px-2.5 py-1 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/30">{pending} review</span>}
                  {revision > 0  && <span className="px-2.5 py-1 rounded-full bg-orange-500/20 text-orange-300 border border-orange-500/30">{revision} revision</span>}
                  {done > 0      && <span className="px-2.5 py-1 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">{done} done</span>}
                  {escalated > 0 && <span className="px-2.5 py-1 rounded-full bg-rose-500/20 text-rose-300 border border-rose-500/30">{escalated} escalated</span>}
                  {cancelled > 0 && <span className="px-2.5 py-1 rounded-full bg-white/10 text-white/40 border border-white/10">{cancelled} cancelled</span>}
                </div>
              </div>

              {/* Productivity metrics — dark glass cards */}
              <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="bg-blue-500/10 rounded-xl px-3 py-2.5 border border-blue-500/20">
                  <p className="text-blue-400 text-xs mb-0.5 font-medium">Completion Rate</p>
                  <p className="text-white/90 font-bold text-lg leading-none">{completionRate}%</p>
                ...
                <div className="bg-amber-500/10 rounded-xl px-3 py-2.5 border border-amber-500/20">
                  <p className="text-amber-400 text-xs mb-0.5 font-medium">Open Tasks</p>
                  <p className="text-white/90 font-bold text-lg leading-none">{openTasks}</p>
                  <p className="text-amber-400/70 text-xs mt-1">of {total} total</p>
                </div>
                <div className="bg-violet-500/10 rounded-xl px-3 py-2.5 border border-violet-500/20">
                  <p className="text-violet-400 text-xs mb-0.5 font-medium">Avg Follow-ups</p>
                  <p className="text-white/90 font-bold text-lg leading-none">{avgFollowups}</p>
                  <p className="text-violet-400/70 text-xs mt-1">per completed task</p>
                </div>
                <div className="bg-emerald-500/10 rounded-xl px-3 py-2.5 border border-emerald-500/20">
                  <p className="text-emerald-400 text-xs mb-0.5 font-medium">Completed</p>
                  <p className="text-white/90 font-bold text-lg leading-none">{done}</p>
                  <p className="text-emerald-400/70 text-xs mt-1">{escalated > 0 ? `${escalated} escalated` : "no escalations"}</p>
                </div>
              </div>
            </div>

            {/* Per-employee filter + sort bar */}
            <div className="px-4 py-2.5 border-b border-white/[0.06] bg-white/[0.02] flex flex-wrap items-center gap-2">
                  ...
                  return (
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-lg text-xs font-medium border transition-all flex items-center gap-1 ${
                        isActive ? "bg-indigo-600 border-indigo-500 text-white shadow-sm" : fc.tab
                      }`}
                    >
                      {f.label}
                      {cnt > 0 && (
                ...
                {([ ["status", "Status"], ["date_desc", "Newest"], ["date_asc", "Oldest"] ] as [SortBy, string][]).map(([k, l]) => (
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-lg text-xs font-medium border transition-all ${
                      empSort === k ? "bg-white/[0.15] border-white/20 text-white" : "bg-white/[0.04] border-white/[0.08] text-white/40 hover:bg-white/[0.08] hover:border-white/[0.14]"
                    }`}
                  >
                    {l}
                  </button>
...
function NewTaskModal({ onClose, onCreated, toast, initialUsers }: {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40 flex items-start justify-center p-4 overflow-y-auto">
      <div className="bg-[#0d1b2e]/95 backdrop-blur-xl border border-white/10 rounded-3xl w-full max-w-lg anim-pop my-4 shadow-2xl shadow-black/60">
        <div className="flex items-center justify-between p-6 border-b border-white/[0.08]">
          <h2 className="text-lg font-semibold text-white/90">Assign New Task</h2>
          <button onClick={onClose} className="w-8 h-8 rounded-full bg-white/[0.08] hover:bg-white/[0.14] text-white/50 flex items-center justify-center transition-all hover:rotate-90 duration-200">
        ...
        <form onSubmit={submit} className="p-6 space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium text-white/50 mb-2">Assign to <span className="text-white/30">(select one or more)</span></label>
            <div
              className="relative"
            ...
            >
              ...
              <div
                ...
                className={`flex flex-wrap items-center gap-1.5 min-h-[46px] bg-white/[0.07] border rounded-xl px-3 py-2 cursor-text transition-all ${
                  pickerOpen ? "border-indigo-400/50 ring-2 ring-indigo-400/20" : "border-white/10 hover:border-white/20"
                }`}
              >
                {selectedIds.map(id => {
                ...
                <input
                  ...
                  className="flex-1 min-w-[120px] bg-transparent border-none outline-none text-sm text-white/80 placeholder-white/25 py-0.5"
                />
              </div>

              {/* Dropdown — all users, toggle to add/remove */}
              {pickerOpen && (
                <div className="absolute left-0 right-0 top-full mt-1 z-10 max-h-44 overflow-y-auto bg-[#0d1b2e]/98 border border-white/10 rounded-xl shadow-xl divide-y divide-white/[0.05] hide-scrollbar anim-pop">
                  {filteredUsers.length === 0 && <p className="text-sm text-white/30 p-3 text-center">No members found</p>}
                  {filteredUsers.map(u => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left px-4 py-2.5 text-sm flex items-center justify-between transition-colors ${sel ? "bg-indigo-500/15 text-indigo-300 font-medium" : "text-white/60 hover:bg-white/[0.05]"}`}
                      >
                        ...
                        {sel ? (
                          ...
                          <span className="text-xs text-white/25">Click to add</span>
                        )}
                      </button>
                    );
                  })}
          ...
          <div>
            <label className="block text-sm font-medium text-white/50 mb-2">
              Due date
              {dueDateInPast && <span className="text-rose-400 font-medium ml-2 text-xs">⚠ Due date is in the past</span>}
            </label>
            <div className="flex gap-2 mb-2">
              {(["open", "custom"] as const).map(m => (
                <button key={m} type="button" onClick={() => setDueDateMode(m)}
                  className={`px-4 py-1.5 rounded-lg text-sm font-medium border transition-all ${dueDateMode === m ? "bg-indigo-600 border-indigo-500 text-white shadow-md shadow-indigo-500/20" : "bg-white/[0.05] border-white/10 text-white/50 hover:border-white/20 hover:text-white/70"}`}
                >
            ...
            {dueDateMode === "custom" && (
              <input
                ...
                className={`${INPUT_CLS} ${dueDateInPast ? "border-rose-500/50" : ""}`}
              />
            )}
          </div>
          ...
          {/* Follow-up schedule */}
          <div>
            <label className="block text-sm font-medium text-white/50 mb-2">Follow-up schedule</label>
            <div className="flex gap-2 mb-3">
              {(["scheduled", "none"] as const).map(m => (
                <button key={m} type="button" onClick={() => setFollowupMode(m)}
                  className={`px-4 py-1.5 rounded-lg text-sm font-medium border transition-all ${followupMode === m ? "bg-indigo-600 border-indigo-500 text-white shadow-md shadow-indigo-500/20" : "bg-white/[0.05] border-white/10 text-white/50 hover:border-white/20 hover:text-white/70"}`}
                >
            ...
            {followupMode === "none" ? (
              <div className="flex items-center gap-2.5 bg-white/[0.04] border border-white/[0.08] rounded-xl px-4 py-3 text-sm text-white/40">
                <svg className="w-4 h-4 shrink-0 text-white/30" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 8v4M12 16h.01"/></svg>
              ...
              <div className="space-y-2">
                  ...
                  return (
                    <div key={i} className="flex gap-2 items-end">
                      <div className="flex-1">
                        <p className="text-xs text-white/30 mb-1 flex items-center gap-2 flex-wrap">
                          <span>{ordinal} Follow-up</span>
                          {isPast && <span className="text-rose-400 font-medium">⚠ This time is already in the past</span>}
                          {isAfterDue && !isPast && <span className="text-rose-400 font-medium">⚠ After due date</span>}
                          {isOutOfOrder && !isPast && !isAfterDue && <span className="text-amber-400 font-medium">⚠ Earlier than previous — will auto-sort</span>}
                        </p>
                        <input
                          type="datetime-local"
                          value={f}
                          ...
                          className={`${INPUT_CLS} ${hasFieldWarning ? "border-rose-500/50" : ""}`}
                        />
                      </div>
                      {followups.length > 1 && (
                        <button type="button" onClick={() => removeFollowup(i)}
                          className="w-9 h-9 rounded-lg bg-rose-500/10 border border-rose-500/25 text-rose-400 hover:bg-rose-500/20 flex items-center justify-center transition-all shrink-0">
                          <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M18 6 6 18M6 6l12 12" /></svg>
                        </button>
                      )}
                    </div>
                ...
                <button type="button" onClick={addFollowup}
                  className="inline-flex items-center gap-1.5 text-xs font-medium text-indigo-300 hover:text-indigo-200 bg-indigo-500/10 border border-indigo-500/25 hover:border-indigo-500/40 px-3 py-1.5 rounded-lg transition-all">
          ...
          <div className="flex gap-3 pt-1">
            <button type="button" onClick={onClose} className="flex-1 bg-white/[0.07] border border-white/10 hover:bg-white/[0.12] text-white/70 font-medium text-sm py-3 rounded-xl transition-all active:scale-[0.98]">
              Cancel
            </button>
            <button type="submit" disabled={!selectedIds.length || !taskText.trim() || (followupMode === "scheduled" && filledFollowups.length === 0) || hasWarning || submitting}
              className="flex-1 bg-gradient-to-r from-blue-500 via-indigo-500 to-violet-600 hover:from-blue-600 hover:via-indigo-600 hover:to-violet-700 hover:-translate-y-0.5 disabled:opacity-50 text-white font-semibold text-sm py-3 rounded-xl shadow-lg shadow-indigo-500/25 transition-all active:scale-[0.98]">
...
export default function DashboardClient({ initialTasks, userEmail }: {
  ...
  return (
    <main className="min-h-screen relative text-white overflow-x-hidden">
      {/* Fixed dark space background with glow orbs */}
      <div className="fixed inset-0 -z-10 bg-[#040b18]">
        <div className="pointer-events-none absolute -top-48 left-1/4 w-[56rem] h-[56rem] rounded-full bg-blue-600/12 blur-[140px]" />
        <div className="pointer-events-none absolute bottom-0 right-0 w-[44rem] h-[44rem] rounded-full bg-indigo-700/12 blur-[120px]" />
        <div className="pointer-events-none absolute top-1/2 -left-24 w-[32rem] h-[32rem] rounded-full bg-violet-600/8 blur-[100px]" />
      </div>
      ...
      <div className="relative max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-6">
          {[
            ...
            { label: "Done",         count: counts.completed,          color: "text-emerald-400" },
            { label: "Escalated",    count: counts.escalated,          color: "text-rose-400" },
          ].map((s, i) => (
        ...
        {/* Filter tabs — dark-friendly color-coded */}
        ...
        <div className="flex items-center gap-2 mb-5">
          <span className="text-xs text-white/30 font-medium shrink-0">Sort:</span>
          {filter === "all" && (
            <button onClick={() => setSortBy("status")} className={`text-xs px-3 py-1.5 rounded-lg border font-medium transition-all ${sortBy === "status" ? "bg-indigo-600 border-indigo-500 text-white" : "bg-white/[0.05] border-white/[0.08] text-white/40 hover:bg-white/[0.09] hover:text-white/70"}`}>
              By Status
            </button>
          )}
          <button onClick={() => setSortBy("date_desc")} className={`text-xs px-3 py-1.5 rounded-lg border font-medium transition-all ${sortBy === "date_desc" ? "bg-indigo-600 border-indigo-500 text-white" : "bg-white/[0.05] border-white/[0.08] text-white/40 hover:bg-white/[0.09] hover:text-white/70"}`}>
            Newest First
          </button>
          <button onClick={() => setSortBy("date_asc")} className={`text-xs px-3 py-1.5 rounded-lg border font-medium transition-all ${sortBy === "date_asc" ? "bg-indigo-600 border-indigo-500 text-white" : "bg-white/[0.05] border-white/[0.08] text-white/40 hover:bg-white/[0.09] hover:text-white/70"}`}>
            Oldest First
          </button>
        </div>
        ...
        {isEmployeeView ? (
          ...
          <div className="space-y-3">
            {visible.length === 0 && (
              <div className={`text-center text-white/30 py-16 ${CARD} rounded-2xl`}>
